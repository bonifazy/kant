  ...
  сайта www.kant.ru категории товаров 'беговая обувь' 4 оффлайн магазинов в г. Москва.
...
синхронизируя всю базу, вызов с присвоением бренда page = Main('Adidas') работает с таблицами только этого бренда.
  ...
  'Main.manager()' -- Настроенная точка входа, которую можно запускать из командной строки с параметрами, к примеру:
    ...
    (env) laptop:kant user$ ./main.py export json
        -- выгружает карточку товаров с описанием продуктов (csv) и размерным рядом и наличием по каждому
    оффлайн- магазину в Москве (json, xml) в файл с именем 'card' и расширением, соответствующим формату экспорта.
